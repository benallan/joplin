...
USAGE="Usage: Joplin_install_and_update.sh [--auto-restart]"

OPTS=$(getopt -o h --long help,auto-restart -- "$@")
if [[ $? -ne 0 ]]; then
...
while true; do
  case "$1" in
    ...
    --auto-restart)
      AUTO_RESTART=1
      echo "Any running Joplin processes will be killed and restarted after upgrade."
      shift
      ;;
    --)
      shift
...
# Check for any leftover non-option args
if [[ $# -gt 0 ]]; then
  echo >&2 "Invalid options: $*"
  echo >&2 "${USAGE}"
  exit 1
...
if [[ ! -e ~/.joplin/VERSION ]] || [[ $(<~/.joplin/VERSION) != "$version" ]]; then
  ...
  if [[ $desktop =~ .*gnome.*|.*kde.*|.*xfce.*|.*mate.*|.*lxqt.*|.*unity.*|.*X-Cinnamon.* ]]; then
    : "${TMPDIR:=/tmp}"
  ...
  #-----------------------------------------------------
  # Restart any previous Joplin processes
  #-----------------------------------------------------
  if [[ "$AUTO_RESTART" -eq "1" ]]; then
